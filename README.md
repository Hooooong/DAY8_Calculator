...
1. 사칙연산
    ...
    1. 모든 숫자와 연산자는 `ArrayList` 에 연산 순으로 담겨 있다.

    2. `ArrayList` 의 홀수 번째에만 연산자가 있으므로 `for` 문을 통해 1, 3, 5... 번째를 찾는다.

    3. `*` 과 `/` 일 때의 index를 통해 index-1, index+1 의 값들을 계산한다.

    4. 연산을 한 숫자를 `ArrayList` 에 index-1 에 넣고, index 번째 값을 2번 지운다.

    5. 2 ~ 4 번을 `+`와 `-` 일 때로 변경한 `for` 문을 반복한다.
...
3. 괄호 추가
    ...
    1. `(` 의 index 와 `)` 의 index 사이의 숫자와 연산자를 `for`을 통해 별도 ArrayList 에 담는다.

    2. 별도 ArrayList 에 담은 숫자와 연산자들을 `사칙연산`의 계산식을 통해 계산한다.

    3. 모든 괄호의 계산이 끝난 후 기존 ArrayList `(` 의  index에 값을 넣어준다.

    4. `)`의  index - `(` 의 index 크기 만큼 기존 ArrayList에 값들을 지워준다.

    5. `while` 문을 통해 괄호가 있을 때 계산을 반복적으로 이룬다.  

    6. `while` 문이 종료되고, 그 이후 `사칙연산` 을 추가적으로 한다.
